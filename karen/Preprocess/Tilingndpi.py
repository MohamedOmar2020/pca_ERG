...
def tiling_qualified_separate_2_5x(svspath,targetpath,tilesize,stride, tissuepct_value=0.8):
    ...
    m=round(float(slide.properties[openslide.PROPERTY_NAME_MPP_X]), 2)//0.23
    level0=int(40/m)
    ...
    for w in range(step_num_w):
        for h in range(step_num_h):
            ...
            if filter.tissue_pct(sub_img) > tissuepct_value: #and filter.h_score(sub_img)>h_value:
                ...
                tile_coord.append((x, y))
                tile = np.array(slide.read_region((x, y), 0,(int(tilesize*downscale_factor), int(tilesize*downscale_factor))))[:, :, :3]
                if downscale_factor>1:
                    tile = resize(tile, (tilesize, tilesize, 3),anti_aliasing=True, preserve_range=True).astype(np.uint8)
