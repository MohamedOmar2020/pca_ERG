...
# rename
metadata['label'].replace('ERG_NEGATIVE', '0', inplace=True)
metadata['label'].replace('ERG_POSITIVE', '1', inplace=True)
...
ERG = pd.DataFrame({'label':metadata['label'], 'PatientID':metadata['slide_id']})

print(ERG['PatientID'])

################################################
## Get the paths for the tiled WSIs
tile_dir = r'/athena/marchionnilab/scratch/lab_data/Mohamed/pca_outcome/data/tiles_karen_NatHist/10x/'
...
# Save to disk
MetaData_training.to_csv('objs/karen/MetaData_training_ERG_NatHist_10x_test.csv')
