...
metadata['label'] = metadata['ETV1status']
...
ETV1 = pd.DataFrame({'label':metadata['label'], 'case_id':metadata['patient_id']})

# replace the '.' with '-'
ETV1['case_id'] =  [i.replace('.', '-') for i in ETV1['case_id']]
...
## Add labels
MetaData_clam = pd.merge(MetaData_clam, ETV1, left_on='case_id', right_on='case_id')
...
# Save to disk
MetaData_clam.to_csv('dataset_csv/pca_ETV1.csv')
