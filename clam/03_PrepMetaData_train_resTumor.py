...
metadata['label'] = metadata['residual_tumor']
metadata['label'].value_counts()

metadata = metadata[metadata.residual_tumor != 'rx']
metadata = metadata[metadata.residual_tumor != 'r2']

## Extract the important clinical labels eg. Gleason grade
ResTumor = pd.DataFrame({'label':metadata['label'], 'case_id':metadata['id']})

# replace the '.' with '-'
ResTumor['case_id'] =  [i.replace('.', '-') for i in ResTumor['case_id']]
...
## Add labels
MetaData_clam = pd.merge(MetaData_clam, ResTumor, left_on='case_id', right_on='case_id')
...
# Save to disk
MetaData_clam.to_csv('dataset_csv/pca_ResTumor.csv')
