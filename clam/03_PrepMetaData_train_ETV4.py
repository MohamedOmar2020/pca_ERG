...
metadata['label'] = metadata['ETV4status']
metadata['label'].value_counts()

# rename
metadata['label'].replace('ETV4 NEGATIVE', 'wt', inplace=True)
metadata['label'].replace('ETV4 POSITIVE', 'fusion', inplace=True)

## Extract the important clinical labels eg. Gleason grade
ETV4 = pd.DataFrame({'label':metadata['label'], 'case_id':metadata['patient_id']})

# replace the '.' with '-'
ETV4['case_id'] =  [i.replace('.', '-') for i in ETV4['case_id']]
...
## Add labels
MetaData_clam = pd.merge(MetaData_clam, ETV4, left_on='case_id', right_on='case_id')
...
# Save to disk
MetaData_clam.to_csv('dataset_csv/pca_ETV4.csv')
